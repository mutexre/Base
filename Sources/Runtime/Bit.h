#ifndef header_DB53AF4CD872
#define header_DB53AF4CD872
...
{
    template <typename T>
    inline bool getBit(T a, u1 b) {
        return (a & (1 << b)) ? true : false;
    }

    template <typename T>
    inline void setBit(T& a, u1 b, bool val) {
        if (val) a |= (1 << b);
        else a &= ~(1 << b);
    }

    template <typename T>
    void printBits(T a, u1 n = 8 * sizeof(T))
    {
        for (u1 b = n; b > 0; b--)
            printf("%c", getBit(a, b - 1) ? '1' : '0');
    }

    static u1 reverseBitsTable[256 * sizeof(u1)] = {
    ...
    inline u1 reverseBits(u1 a) {
        return reverseBitsTable[a];
    }

    inline u2 reverseBits(u2 a) {
        return u2(reverseBits(u1(a & 0x00FF)) << 8) | u2(reverseBits(u1((a & 0xFF00) >> 8)));
    }

    inline u4 reverseBits(u4 a) {
        return u4(reverseBits(u2(a & 0x0000FFFF)) << 16) | u4(reverseBits(u2((a & 0xFFFF0000) >> 16)));
    }
    ...
    inline u1 nlz(u4 a)
    {
        ...
        b = a >> 16; if (b) { n -= 16; a = b; }
        b = a >> 8;  if (b) { n -= 8;  a = b; }
        b = a >> 4;  if (b) { n -= 4;  a = b; }
        b = a >> 2;  if (b) { n -= 2;  a = b; }
        b = a >> 1;  if (b) return n - 2;
    ...
    template <typename T>
    T distributeBits(T src, T pattern, u1 n)
    {
        T dst = 0;

        u1 i, j;
        for (i = 0, j = 0; i < n; i++) {
            while (!getBit(pattern, j)) j++;
            setBit(dst, j++, getBit(src, i));
        }
